...
data <- add_categorical_choices(data, cut_points, categories)
...
#plot.CDDM_choiceData(data,par=par)
#~~~~~~~~~~ Fit models ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
############################################################
...
for(i in 1:nIter){
    ...
    arc_choice <- output$arc_choice
    rt <- output$rt
    sumStats <- get_summaryStats(angular_vector = arc_choice, rt_vector = rt)
